...
- iOS（App Store/iTunes）
  ...
  - App Store（非国区）：`Surge` 、`Potatso Lite` 、`Wingy`、`Quantumult`、`squirrelVPN`、`nthLink`、`banana`、`gitspeed`、`flyrouter`、`puffin`、`mumevpn`、`anyconnect`、`Kitsunebi`、`Outline App`等.
...
- [hostwinds](https://www.hostwinds.com/vps/unmanaged-linux/){:target=>"_blank"}
...
SS 启动后若发现一些网站打不开, 估计是 DNS 被污染了
路由 SS 设置选择其他 DNS 服务模式；
电脑设置 DNS 自动获取路由 ip。检查 hosts 是否有错误规则。
电脑运行 cmd 输入【ipconfig /flushdns】, 清理浏览器缓存。

### V2Ray换端口

这次V2Ray不能使用，实际上是根据V2Ray客户端本地端口封锁的。将原端口改为其他任一端口，只要没有提示与其他应用有冲突即可。
V2RayN“设置”-“参数设置”-“Core:基础设置”，这里的“本地监听端口”默认为 10808，改成其他数字，比如1080.
...
### Clash的允许局域网连接（Allow LAN）

此功能可以共享网络环境给同一局域网的其他设备。这里要注意的是手机和电脑都连接同一个wifi（以一台电脑和一部手机为例），而不是手机连接电脑开的热点。
打开Clash for Windows的Allow LAN开关，然后鼠标停在上这个选项上，会显示IP地址。
手机打开连接的wifi设置——高级设置——代理服务器选择手动——代理主机名填刚才显示的IP，代理端服务器端口7890——保存

### Clash选择代理出现Could not switch to this profile

我的解决办法是先退出Clash，然后删除C:\Users\Administrator的.config文件夹，重启Clash，会自动重新生成.config，重新下载订阅后就可以了。
C:\Users\*********\.config\config.yaml

### Clash的Mixin功能

一般机场的订阅配置里面会自带路由规则，但是不太全，可以用这个大佬的 <https://github.com/Loyalsoldier/clash-rules>
使用Mixin功能，把路由规则集改成这个大佬的

### TAP模式和TUN模式

非系统代理应用通过Clash for Windows上网，电脑上非浏览器软件通过Clash上网
浏览器之类的应用都是使用系统代理的，一些非系统代理应用，可以通过设置被Clash for Windows接管。目前有两种模式，TAP模式和TUN模式，更推荐使用TUN模式

【TUN模式】
1、要开启TUN先要安装Service Mode
点击General中ServiceMode右边Manage，在打开窗口中安装服务模式，安装完成应用会自动重启，Service Mode右边地球图标变为绿色（点击manage会出现Current status:Active）即安装成功
...
启动 TAP 模式：点击settings——Profile Mixin——YAML，点击edit，输入下面的内容，点击保存
...
了解最新网络信息安全资讯请看[网安新闻](https://hcntomoon.github.io/posts/%E7%BD%91%E5%AE%89%E6%96%B0%E9%97%BB){:target="_blank"} ，了解翻墙后看什么网站请看[翻墙后看什么？](https://hcntomoon.github.io/posts/%E7%BF%BB%E5%A2%99%E5%90%8E%E7%9C%8B%E4%BB%80%E4%B9%88){:target="_blank"}
